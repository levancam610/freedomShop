...
const headerLink = [
    ...
    {title: "Sản phẩm", url: "/admin/product/list", active: false},
...
class Header extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            orderOptions:[
                ...
                {column: "rate",label: "Sao"},
                {column: "price",label: "Giá tiền"},
            ],
        }
    }
    changePageSize = (size) => {
        this.state.selectedPageSize = size;
        this.props.getListProduct(size, 1,this.props.data.orderBy, this.props.data.CategorySelect);
    }
    changeFilter = (filter) => {
        this.state.selectedFilterOptions = filter;
        this.props.getListProduct(this.props.data.selectPageSize, 1, this.props.data.orderBy, filter.value);
    }
    changeOrderBy = (order) => {
        this.state.selectedOrderOption = order;
        this.props.getListProduct(this.props.data.selectPageSize, 1, order.column, this.props.data.CategorySelect);
    }
    componentWillMount(){
    ...
    render(){
        ...
        if(this.props.data.Categories.length>0){
            ...
            this.state.filterOptions = [{label: messages["label.all"], value: 0}, ...this.state.filterOptions];
        }
